        ...
        <el-form :model="ruleForm" :rules="rules" ref="insertAdmin">
            <el-row :gutter="0" v-for="(item,i) in labels" :key="JSON.stringify(item)+i">
                <el-col v-for="(jtem,j) in item" :span="jtem.len" :key="JSON.stringify(jtem)+j">
                    ...
                    <yg-upload-file v-if="jtem.yt == 'file'" :formLabelWidth="jtem.labelWidth" :label="jtem.label" :prop="jtem.prop||jtem.name"
                                   :value="ruleForm[jtem.name]" :fileUrl="options[jtem.name]"
                    ...
                    <div v-else-if="jtem.yt == 'textarea'" :style="{paddingLeft:jtem.labelWidth}" class="remarkBox clear"><label style="top: 10px">{{jtem.label}}</label><textarea :style="{height:jtem.height?jtem.height:'36px'}" :value="ruleForm[jtem.name]"></textarea></div>
                </el-col>
            </el-row>
        </el-form>
    ...
    export default {
        data() {
            return {
                ...
                labels: [
                    [
                        {label: '专家头像',labelWidth:"100px", name: 'avatar', yt: 'img', len: 11},
                        {yt: 'empty', len: 2},
                        {label: '专家视频',labelWidth:"100px",  name: 'video', yt: 'file', len: 11},
                    ],
                    [
                        {label: '专家编码',labelWidth:"100px",  name: 'type', yt: 'select', placeholder: '请选择类型', len: 11},
                        {yt: 'empty', len: 2},
                        {label: '专家类别',labelWidth:"100px",  name: 'type', yt: 'select', placeholder: '请选择类型', len: 11},
                    ],
                    [
                        {label: '专家姓名',labelWidth:"100px",  name: 'categoryname', yt: 'input', len: 11},
                        {yt: 'empty', len: 2},
                        {label: '手机号',labelWidth:"100px",  name: 'categoryname', yt: 'input', len: 11},
                    ],
                    [
                        {label: '所在单位',labelWidth:"100px",  name: 'type', yt: 'select', placeholder: '请选择类型', len: 11},
                        {yt: 'empty', len: 2},
                        {label: '职称',labelWidth:"100px",  name: 'type', yt: 'select', placeholder: '请选择类型', len: 11},
                    ],
                    [
                        {label: '展示位置',labelWidth:"100px",  name: 'type', yt: 'select', placeholder: '请选择类型', len: 11},
                        {yt: 'empty', len: 2},
                        {label: '状态',labelWidth:"100px",  name: 'devicestatus', yt: 'radio', len: 11},
                    ],
                    [
                        {label: '特长',labelWidth:"100px",  name: 'categoryname', yt: 'textarea', len: 24},
                    ],
                    [
                        {label: '专家介绍',labelWidth:"100px", height:"68px", name: 'categoryname', yt: 'textarea', len: 24},
                    ],
                    [
